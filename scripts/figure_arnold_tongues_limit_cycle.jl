...
## PANEL A ===
df = load_data("./outputs/arnold_tongues/van_der_pol_limit_cycle/arnold_tongue_ode.csv")
fig, ax = subplots()
...
for (iσ, σ) = enumerate(σ_array)
    ...
    df = load_data("./outputs/arnold_tongues/van_der_pol_limit_cycle/arnold_tongue_σ=$(σ).csv")
    for (in, n) = enumerate(n_array)
        ...
        fig.tight_layout()
        save_figure(fig, "./figures/arnold_tongues/van_der_pol_limit_cycle/arnold_tongue_n=$(n)_σ=$(σ).svg")
        close(fig)
...
ax.set_title("Limit cycle oscillator", pad=0, loc="left")
ax.set_xlabel("log\$_{10}\$(Noise intensity)", labelpad=0)
ax.set_ylabel("Average phase coherence", labelpad=0)
fig.tight_layout(pad=0.1)
save_figure(fig, "./figures/arnold_tongues/van_der_pol_limit_cycle.svg")
