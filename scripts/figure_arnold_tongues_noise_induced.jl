...
## PANEL A ===
df = load_data("./outputs/arnold_tongues/van_der_pol_noise_induced/arnold_tongue_ode.csv")
fig, ax = subplots()
...
## ==
σ_array = [0.005, 0.01, 0.02, 0.03, 0.05, 0.1]
n_array = [1, 10, 100, 1000]
phase_coherence_matrix = Matrix{Float64}(undef, length(σ_array), length(n_array))
for (iσ, σ) = enumerate(σ_array)

    df = load_data("./outputs/arnold_tongues/van_der_pol_noise_induced/arnold_tongue_σ=$(σ).csv")
    for (in, n) = enumerate(n_array)
        ...
        fig.tight_layout()
        save_figure(fig, "./figures/arnold_tongues/van_der_pol_noise_induced/arnold_tongue_n=$(n)_σ=$(σ).svg")
        close(fig)
...
## ==
fig, ax = subplots(figsize=(6.5, 2.25))
...
ax.legend(edgecolor="black", framealpha=1.0, ncol=1)
ax.set_title("Noise-induced oscillator", pad=0, loc="left")
ax.set_xlabel("log\$_{10}\$(Noise intensity)", labelpad=0)
ax.set_ylabel("Average phase coherence", labelpad=0)
fig.tight_layout(pad=0.1)
save_figure(fig, "./figures/arnold_tongues/van_der_pol_noise_induced.svg")
